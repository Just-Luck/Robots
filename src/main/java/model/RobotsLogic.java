...
public class RobotsLogic extends Observable
{
    ...
    public void moveRobot()
    {
        ...
        final double newAngle = RobotsMath.asNormalizedRadians(robot.getDirection() + robot.getAngularVelocity() * dt);

        final double dx = robot.getVelocity() / robot.getAngularVelocity() * (Math.sin(newAngle) - Math.sin(robot.getDirection()));
        final double dy = robot.getVelocity() / robot.getAngularVelocity() * (Math.cos(newAngle) - Math.cos(robot.getDirection()));
